...
describe('payload-self-manual v0.4.0 ecosystem & plugins introspection contract', () => {
	it('inspects plugins from Payload configuration including registered docs and custom metadata', () => {
		...
		const selfManual = report.plugins.find(
			(p) => p.name === '@nan0web/payload-self-manual' || p.id === 'self-manual'
		)
		...
		const selfStorage = report.plugins.find(
			(p) => p.name === '@nan0web/payload-self-storage' || p.id === 'self-storage'
		)
		...
		// Also verify admin.custom.selfStorage detection
		const adminCustomReport = inspectPlugins({
			admin: {
				custom: {
					selfStorage: {
						enabled: true,
		...
		const storageFromAdmin = adminCustomReport.plugins.find((p) => p.id === 'self-storage')
		assert.ok(storageFromAdmin, 'self-storage should be detected from admin.custom.selfStorage')
		assert.equal(storageFromAdmin.version, '0.2.0')
		assert.equal(storageFromAdmin.status, 'active')
		assert.equal(storageFromAdmin.hasDocs, true)
	})
		...
		const mockConfig = {
			custom: {
				selfManualDocs: [
					{
						id: 'plugin-a',
						source: '@nan0web/payload-plugin-a',
						title: 'Plugin A',
						docsDir: '/docs/a',
					},
				],
